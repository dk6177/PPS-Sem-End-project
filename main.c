...
int main()
{
	...
	while(1){
		...
		printf("\n\n\n\n\t\t\t\tEnter s for STONE, p for PAPER, z for SCISSOR and e for EXIT\n\t\t\t\t\t\t\t");
		...
		if (you == 'e'){
			...
			fprintf(counter,"\n your wins on last playtime were: %d",w);
			fprintf(counter,"\n your loses on last playtime were: %d",l);
			fprintf(counter,"\n your draws on last playtime were: %d\n\n\n\n\n",d);
			fclose(counter);
			exit(0);
		}
		else if (result == -1) {
			...
			fprintf(score,"\n you lost... better lluck next time :( \n");
			l++;
		}
